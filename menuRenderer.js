// File: menuRenderer.js (منطق العرض - مصحح ونهائي)
...
export function renderMenu(menuData, currentBranchId) {
    ...
    menuData.forEach(section => {
        ...
        section.items.forEach(item => {
            ...
            // تطبيق الخصم إذا كان موجوداً لهذا الفرع
            if (item.branchDiscounts && item.branchDiscounts[currentBranchId]) {
                displayPrice = item.branchDiscounts[currentBranchId];
                priceText = `<span class="old-price">${item.basePrice.toFixed(2)}</span> <span class="discount-price">${displayPrice.toFixed(2)}</span> ريال`;
            }
            ...
            // ⭐️ التصحيح النهائي: الاعتماد مباشرة على المتغيرين (item) و (section) المتاحين في السياق (Closure).
            addButton.addEventListener('click', () => {
                const sectionIndex = menuData.indexOf(section); 
                // تمرير كائن (item) مباشرة إلى دالة showModal
                window.showModal(item, sectionIndex); 
            });
